...
public class VéhiculeController {
	...
	public ModelMap Véhicule(@PageableDefault(size = 7) Pageable pageable,
			@RequestParam(name = "value", required = false) String value, Model model) {
		// si la valeur passée en argument est different de null c'est à dire on va
		// faire la recherche
	...
	public String enregistrer(@Valid @ModelAttribute("vehicule") VehiculeRegistrationDto vehiculedto,
			BindingResult errors, SessionStatus status, Model model, RedirectAttributes redirectAttrs) {
	...
	public String delete(@ModelAttribute("vehicule") Vehicule vehicule, BindingResult result, ModelMap m)
